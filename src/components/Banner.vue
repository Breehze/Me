...
<template>       
        <div @click = "onHover" @mouseenter = "onHover" @mouseleave = "onLeave" :class = hoveredClass  class="transition-all duration-500 ease-in-out relative  bg-black border-black border-2  h-5/6  w-80 text-white m-5 snap-center">
            ...
            <div v-if="showText" class="absolute top-32  left-0 ml-5 w-48 xl:w-80 ">
                <slot name = "career"></slot>
            </div>
            
            <div v-if="showText" class="absolute top-32  left-0 ml-3">
                <slot name = "skills"></slot>
            </div>

            <div v-if="showText" class="absolute top-48  right-0 ">
                <slot name = "projects"></slot>
            </div>
            
            <div v-if="showText" class="absolute top-48  left-0 ">
                <slot name = "projectMenu"></slot>
            </div>
            
            
            
            <div v-if="showText" class="transition-all duration-1000 animate-fade-right ease-in-out absolute -bottom-0 right-1 w-4/5 m-9 xl:text-xl">
